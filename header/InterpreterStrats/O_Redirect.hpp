#ifndef __O_REDIRECT_HPP__
#define __O_REDIRECT_HPP__
...
class O_Redirect : public StratDecorator{
		...
		O_Redirect(InterpreterStrat* Base){this->Base = Base;}
			...
			if (READING_NEXT_TOKEN == false){		
			    ...
			    calls.push_back("15 REDIRECT"); //set the command decorator to O_REDIRECT
			    this->READING_NEXT_TOKEN = true;
			} else {
			    this->READING_NEXT_TOKEN = false;
			    calls.push_back("15 REDIRECT_TARGET"); //set the command decorator to O_REDIRECT_TARGET
